...
def healed(e):
    player = players.from_userid(e['userid'])
    if player.hasPerk(emt.name):
        players.from_userid(e['subject']).health += emt.effect * player.getPerkLevel(emt.name)
    if not player.is_bot():
        if e['userid'] != e['subject']:
            giveXP(e['userid'], healingTeamMateXP, "healing a friend!")
        else:
            pass
        
@Event('survivor_rescued')
def rescuedPlayer(e):
    if not Player(e['rescuer']).is_bot():
        giveXP(e['rescuer'], rescueXP, "saving a friend!")
